...
singleBanded(Nx-2, A, -4, 1);
//printMatrix(A, Nx-2, Nx-2);
for (int j = 1; j < Ny-1; j++) {
    for (int i = 2; i < Nx-2; i++) {
        u_j[i-1] = mu1/h/h*u[ts+i+Nx*j];
        u_jp1[i-1] = mu1/h/h*(u[ts+i+Nx*(j+1)]+u[ts+i+Nx*(j-1)])+f1(ts+i+Nx*j);
        v_j[i-1] = mu2/h/h*v[ts+i+Nx*j];
        v_jp1[i-1] = mu2/h/h*(v[ts+i+Nx*(j+1)]+v[ts+i+Nx*(j-1)])+f2(ts+i+Nx*j);
    }
    F77NAME(dgemv) ('N', Ny-2, Nx-2, 1, A, Nx-2, u_j, 1, 0, yu, 1);
    F77NAME(dgemv) ('N', Ny-2, Nx-2, 1, A, Nx-2, v_j, 1, 0, yv, 1);
    for (int i = 2; i < Nx-2; i++) {
        u[t2+Nx*(j)+i] = (yu[i-1]+u_jp1[i-1])*dt+u[ts+Nx*(j)+i];
        v[t2+Nx*(j)+i] = (yv[i-1]+v_jp1[i-1])*dt+v[ts+Nx*(j)+i];
    }
    /*u[t2+Nx*(j)+1] = (yu[0]+u_jp1[0])*dt+mu1/h/h*u[ts+Nx*(j)]*dt+u[ts+Nx*(j)];
    u[t2+Nx*(j)+Nx-2] = (yu[Nx-2-1]+u_jp1[Nx-2-1])*dt+mu1/h/h*u[ts+Nx*(j)]*dt+u[ts+Nx*(j)];
    v[t2+Nx*(j)+1] = (yv[0]+v_jp1[0])*dt+mu2/h/h*v[ts+Nx*(j)]*dt+v[ts+Nx*(j)];
    v[t2+Nx*(j)+Nx-2] = (yv[Nx-2-1]+v_jp1[Nx-2-1])*dt+mu2/h/h*v[ts+Nx*(j)]*dt+v[ts+Nx*(j)];*/
}
/*
for (int j = 1; j < (Ny-1); j++) {
    for (int i = 1; i < (Nx-1); i++) {
